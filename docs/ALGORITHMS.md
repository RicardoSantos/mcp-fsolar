...
This section documents all active security controls in `server.ts` and `src/hooks.ts`. Each control can be audited in isolation; `test/security.test.js` provides a live integration test suite for all of them.
...
### `computeAutonomy` — `src/compute.ts`
